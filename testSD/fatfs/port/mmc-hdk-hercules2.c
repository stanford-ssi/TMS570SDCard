...
/*------------------------------------------------------------------------------
  Write and Read a byte on SPI2 interface
 *------------------------------------------------------------------------------*/
...
/*-----------------------------------------------------------------------*/
/* Transmit a byte to MMC via SPI2  (Platform dependent)                  */
/*-----------------------------------------------------------------------*/
...
void xmit_spi(BYTE dat)
{
    ...
    while ((spiREG2->FLG & 0x0200) == 0); // Wait until TXINTFLG is set for previous transmission
    spiREG2->DAT1 = dat | 0x100D0000;    // transmit register address

    while ((spiREG2->FLG & 0x0100) == 0); // Wait until RXINTFLG is set when new value is received
    ui32RcvDat = spiREG2->BUF;  // to get received value
}


/*-----------------------------------------------------------------------*/
/* Receive a byte from MMC via SPI2  (Platform dependent)                 */
/*-----------------------------------------------------------------------*/
...
BYTE rcvr_spi (void)
{
    ...
    while ((spiREG2->FLG & 0x0200) == 0); // Wait until TXINTFLG is set for previous transmission
    spiREG2->DAT1 = 0xFF | 0x100D0000;    // transmit register address
...
// set the SPI2 speed to the max setting
static
void set_max_speed(void)
{
      ...
      spiREG2->FMT0 |= 5 << 8;     // baudrate prescale 10MHz / (1+1) = 5MBit
}
