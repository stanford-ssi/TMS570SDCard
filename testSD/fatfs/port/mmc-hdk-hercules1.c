...
void xmit_spi(BYTE dat)
{
    ...
    while ((spiREG1->FLG & 0x0200) == 0); // Wait until TXINTFLG is set for previous transmission
    spiREG1->DAT1 = dat | 0x100D0000;    // transmit register address

    while ((spiREG1->FLG & 0x0100) == 0); // Wait until RXINTFLG is set when new value is received
    ui32RcvDat = spiREG1->BUF;  // to get received value
}
...
BYTE rcvr_spi (void)
{
    ...
    while ((spiREG1->FLG & 0x0200) == 0); // Wait until TXINTFLG is set for previous transmission
    spiREG1->DAT1 = 0xFF | 0x100D0000;    // transmit register address
...
void set_max_speed(void)
{
      ...
      spiREG1->FMT0 |= 5 << 8;     // baudrate prescale 10MHz / (1+1) = 5MBit
}
